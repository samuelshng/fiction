...
#pragma region Create arbitrary functions
     ...
     *
     * Retrieves truth tables from the source network and creates a new node connected to `children`. For single-output
     * nodes, this copies one function. For multi-output nodes (e.g., half-adders), this copies all output functions in
     * output-pin order to preserve each output signal's semantics.
     *
    ...
    signal clone_node(const block_network& other, const node& source, const std::vector<signal>& children)
    {
        ...
        if (other.is_multioutput(source))
        {
            ...
            functions.reserve(other._storage->nodes[source].data.size() - 2);

            for (auto i = 2u; i < other._storage->nodes[source].data.size(); ++i)
            {
                functions.push_back(other._storage->data.cache[other._storage->nodes[source].data[i].h1]);
            }

            return mockturtle::block_network::create_node(children, functions);
        }

        const auto function = other._storage->data.cache[other._storage->nodes[source].data[2].h1];
        return create_node(children, function);
    }
